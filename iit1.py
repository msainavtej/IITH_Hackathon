        ...
        with open(csv_path, "r") as f:
            ...
            if len(rows) > 1:
                ...
                return {
                    ...
                    "Reason": last[2]
                    # Distracted Time removed
                }
    except Exception:
        pass
    return {"Time": "--", "Focus Score": 0, "Reason": "No data"}
...
def team_data_bridge():
    ...
    global history, last_plot_time, window_history, calibration_status
    try:
        if focus_tracker is None or (focus_tracker_thread is not None and not focus_tracker_thread.is_alive()):
            start_background_trackers()

        is_calibrating, baseline_conf = focus_tracker.get_calibration_status() if focus_tracker else (True, None)
        focus_metrics = read_latest_focus_csv()
        focus_score = focus_metrics["Focus Score"]
        face_conf = f"{focus_score}%"
        current_time = time.time()

        # Set calibration_status for UI
        if is_calibrating:
            calibration_status = "🔄 Calibrating... Please look at the screen"
        else:
            calibration_status = focus_metrics.get("Reason", "")
        ...
        if not is_calibrating:
            if category == "wasting":
                ...
                if elapsed_wasting > 20 and not already_triggered:
                    ...
                    threading.Thread(target=run_intervention, daemon=True).start()
            else:
                wasting_start_time = None
                already_triggered = False
        ...
        if current_time - last_plot_time >= 3:
            history.append({"Time": focus_metrics["Time"], "Productivity": focus_score, "App": category.capitalize()})
            if len(history) > 20: history.pop(0)
            last_plot_time = current_time

    except Exception as e:
        print(f"[team_data_bridge] Error: {e}")
        return pd.DataFrame(history), "Error", "0%", calibration_status
    finally:
        try:
            return pd.DataFrame(history), active_app, face_conf, calibration_status
        except Exception:
            return pd.DataFrame(history), "Error", "0%", calibration_status
...
with gr.Blocks() as demo:
    gr.Markdown("# Autonomous Productivity Hub")

    with gr.Row():
        active_app = gr.Textbox(label="Active Window")
        face_conf = gr.Textbox(label="Focus Score (%)")
        calibration_label = gr.Label(label="Status") # total_time removed
    ...
    timer.tick(
        fn=team_data_bridge,
        outputs=[plot, active_app, face_conf, calibration_label]
    )
